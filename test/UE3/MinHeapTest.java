...
class MinHeapTest {
	...
	void constructor() {
		...
		final HeapError error = findErrorInHeap(longs);
		assertNull(error, String.format("Heap structure violated at %s\n%s", error, heap.toString()));
	}

	private HeapError findErrorInHeap(final Long[] heap) {
		for (int i = 0; i < heap.length / 2 - 1; i++) {
			final int left = leftChild(heap, i), right = rightChild(heap, i);
			if (heap[i].compareTo(heap[left]) > 0)
				return new HeapError(i, left, heap[i], heap[left]);
			if (heap[i].compareTo(heap[right]) > 0)
				return new HeapError(i, right, heap[i], heap[right]);
		}
		return null;
	}
	...
	void vorlesungsFolien() {
		...
		if (MATCH_LECTURE_EXACTLY) {
			...
			assertArrayEquals(lecture, longs);
		}
	}
	...
	private static class HeapError {
		private final int i1, i2;
		private final Long l1, l2;

		private HeapError(final int i1, final int i2, final Long l1, final Long l2) {
			this.i1 = i1;
			this.i2 = i2;
			this.l1 = l1;
			this.l2 = l2;
		...
		public String toString() {
			return String.format("HeapError: heap[%d]=%s < heap[%d]=%s", i2, l2, i1, l1);
		}
	}
}